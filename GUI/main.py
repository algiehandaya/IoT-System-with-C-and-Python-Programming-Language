...
class FormPage(QWidget):
    def __init__(self, display_page):
        super().__init__()
        ...
        # Tambahkan kode CSS untuk label error
        ...
        self.button.clicked.connect(self.show_display_page)
    ...
    def show_display_page(self):
        ...
        self.display_page.update_display_text(text_list)
...
class DisplayPage(QWidget):
    def __init__(self, form_page):
        ...
        self.gateway_x = int(width * 1/4)
        self.gateway_y = int(height/2)
        self.node1_x = int(width * 3/4)
        self.node1_y = int(height/2)
        self.node2_x = int(width/2 - 100)
        self.node2_y = int(height * 1/4)
        self.node3_x = int(width/2 + 100)
    ...
    def update_display_text(self, text_list):
        ...
        print(f'RSSI to node 2 : {int(self.rssi_values[1])} - RSSI to node 3 : {int(self.rssi_values[2])} - TH : {int(self.rssi_values[9])}')
    ...
    def paintEvent(self, event):
        ...
        qp.drawText(self.gateway_x-120, self.gateway_y, "GATEWAY")
        ...
        if self.dispText[0] is True: #Display Text Gateway
            ...
            for line in lines:
                qp.drawText(self.gateway_x-130, y, line)
                y += line_spacing
            qp.drawText(self.gateway_x-100, self.gateway_y+90, str(f"RSSI THRESHOLD : {self.rssi_values[9]}"))
        ...
        if self.dispText[3] is True: #Display Text Node 3
            ...
            y = self.node3_y + 80
            for line in lines:
                qp.drawText(self.node3_x-30, y, line)
                y += line_spacing
...
if __name__ == "__main__":
    ...
    form_page = FormPage(DisplayPage(form_page=None))
    display_page = DisplayPage(form_page=form_page)
    form_page.display_page = display_page
