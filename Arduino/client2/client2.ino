...
void loop()
{
    ...
    //Serial.println(suhuDS18B20, 4);
      ...
      if (turn == 2) { //kirim data client node 2
        //Serial.println("Client 2 Waiting Message..");
        waitForAnswer2();   
      ...
      if (turn == 3) { //kirim data client node 2
        //Serial.println("Client 2 Waiting Message Again..");
        waitForAnswer3();
...
if (rsc.status.code!=1){
    ...
    Serial.println(message.temperature);
    nilaiph3 = message.ph;
    nilaitemp3 = message.temperature;
...
void waitForAnswer2() { //fungsi tunggu jawaban dari node
  ...
  struct  Message3 message3 = {nilaiph3, nilaitemp3};
  
  
 // If something available
   if (e220ttl.available()>=1) {
   Serial.println("Message received by Client 2!");
    // read the String message
...
if (rssidbm < 155){
  ...
  if (rc.status.code!=1){
    ...
    else if(kirim == 1 && rc.data == "Client 3 Request Data"){
      //Send Message
...
void waitForAnswer3() { //fungsi tunggu jawaban dari node
 ...
 // If something available
   if (e220ttl.available()>=1) {
   Serial.println("Message received by Client 2 Again!");
    // read the String message
...
if (rssidbm < 155){
  ...
  if (rc.status.code!=1){
    ...
    if (kirim == 3 && rc.data == "Gateway Request Data 2"){
    //Send Message
    #define DESTINATION_ADDL 8
    Serial.println("Sending 2");
    ResponseStatus rs = e220ttl.sendFixedMessage(0, DESTINATION_ADDL, 23, &message2, sizeof(Message2));
